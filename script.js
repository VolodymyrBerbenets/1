...
let selectedCategory = categories[0];
  ...
  // взять все задания с определенном категории
  ...
  <div class="content">
    ...
    <p>${categoryTasks.length} Завдання</p>
  </div>
</div>
<div class="options">
...
`;
  ...
  const renderTasks = () => {
    ...
    // если нет заданий для категории
    if (categoryTasks.length === 0) {
        tasksContainer.innerHTML = `
        <p class="no-tasks">Для цього блоку немає завдань</p>`
    } else {
  ...
  addBtn.addEventListener("click", () => {
    ...
    if(task === "") {
        alert("Будь-ласка напишіть завдання");
    } else {
        const newTask = {
            id : tasks.length + 1,
            task,
